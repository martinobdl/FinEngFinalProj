function [rho_nested,DR_nested] = nestedSimulation(d_hat,d_std,nSim_d,...
                                                   CRsurface,rho)
...
%                 - d_std:     value of empirical standard deviation
%                 - nSim_d:    number of simulation of d for each rho
%                 - CRsurface: struct containing d and rho fields, that
%                              determine a grid on which the field surf
%                              is defined
%                 - rho:       vector of given simulated correlation
% @outputs:       - rho_nested: vector of simulated rho
%                 - d_nested:   vector of simulated d, associated to rho
%
% @functions:     - posteriorDistributionD
%                 - bayesianPrediction
%                 - samplingFromPosterior
%
...
xd_vect = linspace(-4,4,1000);            % vector on which evaluate ccd
f_x_dCR = @(x) normpdf(x,d_vect,d_std);   % density of x given mean(unknown) and std known
...
for i = 1 : nSim_rho                      % cycle over the desired rho
    ...
    d_CRstd = interp2(CRsurface.d,CRsurface.rho,CRsurface.surf,...
                      d_vect,rho(i),'spline');                    % CramerRao std of d parameter
    h_d_CR = posteriorDistributionD(d_hat,d_vect,d_CRstd);        % posterior distribution of d parameter
    xd_pdfCR = bayesianPrediction(xd_vect,d_vect,h_d_CR,f_x_dCR); % density of X conditioned on data
    d_nested(1+(i-1)*nSim_d:i*nSim_d) = samplingFromPosterior(nSim_d,...
                       xd_vect,xd_pdfCR);   % simulated observation from its density
end 
