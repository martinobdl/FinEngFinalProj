...
idiosyncraticRisk = randn(N_sim,N_obligors);   % simulation of idiosyncratic risk
...
RC_LHP = CapitalRequirementNominalLHP(RecRate_mean,DefRate_mean,rho_mean,CL);
    % For Homogeneous Portfolio
RC_HP = CapitalRequirementNominalHP(RecRate_mean,DefRate_mean,rho_mean,CL,N_obligors);
...
disp('Regulatory Capital nominal model, basel correlation')
    % For Large Homogeneous Portfolio
RC_LHP_B = CapitalRequirementNominalLHP(RecRate_mean,DefRate_mean,rho_Basel,CL);
    % For Homogeneus Portfolio
RC_HP_B = CapitalRequirementNominalHP(RecRate_mean,DefRate_mean,rho_Basel,CL,N_obligors);
...
if ~H_d
    fprintf('Normality assumption accpted for d with pvalue of: %.3f\n', pValue_d);
else
    fprintf('Normality assumption rejected for d with pvalue of: %.3f\n', pValue_d);
end

disp('Recovery rate normality, Shapiro-Wilk test')
[H_RR,pValue_RR] = swtest(data.RR);
if ~H_RR
    fprintf('Normality assumption accpted for recovery with pvalue of: %.3f\n',pValue_RR);
else
    fprintf('Normality assumption rejected for recovery with pvalue of: %.3f\n',pValue_RR);
end
  %% B.2 - B.3 Simulations required
   
d_sim_freq  = d_hat + randn(N_sim,1)*d_std;     % simulated gaussian threshold 
DefRate_sim_freq = normcdf(d_sim_freq);                 % simulated default rates

RecRate_sim_freq = RecRate_mean + randn(N_sim,1)*RecRate_std;  % simulated gaussian recovery 
...
RCf_LHP_d     = CapitalRequirementAlternativeLHP(RecRate_mean,DefRate_sim_freq,...
               rho_mean,CL,systematicRisk);  % default rate
RCf_LHP_RR    = CapitalRequirementAlternativeLHP(RecRate_sim_freq,DefRate_mean,...
               rho_mean,CL,systematicRisk);  % recovery rate
RCf_LHP_rho   = CapitalRequirementAlternativeLHP(RecRate_mean,DefRate_mean,...
               rho_sim_freq,CL,systematicRisk); % correlation
RCf_LHP_d_rho = CapitalRequirementAlternativeLHP(RecRate_mean,DefRate_sim_freq,...
               rho_sim_freq,CL,systematicRisk); % default rate and correlation
RCf_LHP_all   = CapitalRequirementAlternativeLHP(RecRate_sim_freq,DefRate_sim_freq,...
               rho_sim_freq,CL,systematicRisk); % all parameters
...
RCf_HP_d     = CapitalRequirementAlternativeHP(RecRate_mean,DefRate_sim_freq,rho_mean,...
      CL,systematicRisk,idiosyncraticRisk); % default rate
RCf_HP_RR    = CapitalRequirementAlternativeHP(RecRate_sim_freq,DefRate_mean,rho_mean,...
      CL,systematicRisk,idiosyncraticRisk); % recovery rate
RCf_HP_rho   = CapitalRequirementAlternativeHP(RecRate_mean,DefRate_mean,rho_sim_freq,...
      CL,systematicRisk,idiosyncraticRisk); % correlation
RCf_HP_d_rho = CapitalRequirementAlternativeHP(RecRate_mean,DefRate_sim_freq,rho_sim_freq,...
      CL,systematicRisk,idiosyncraticRisk); % default rate and correlation
RCf_HP_all   = CapitalRequirementAlternativeHP(RecRate_sim_freq,DefRate_sim_freq,rho_sim_freq,...
...
disp('Large Homogeneous Portfolio')
% Computing Regulatory Capitals simulating different parameters LHP CL
RCf_LHP_B_d    = CapitalRequirementAlternativeLHP(RecRate_mean,DefRate_sim_freq,...
                rho_sim_B_freq,CL,systematicRisk);  % default rate
RCf_LHP_B_RR   = CapitalRequirementAlternativeLHP(RecRate_sim_freq,DefRate_mean,...
                rho_Basel,CL,systematicRisk);        % recovery rate
RCf_LHP_B_d_RR = CapitalRequirementAlternativeLHP(RecRate_sim_freq,DefRate_sim_freq,...
                rho_sim_B_freq,CL,systematicRisk);  % default and recovery
...
disp('Homogeneous Portfolio')
% Computing Regulatory Capitals simulating different parameters HP CL
RCf_HP_B_d    = CapitalRequirementAlternativeHP(RecRate_mean,DefRate_sim_freq,...
   rho_sim_B_freq,CL,systematicRisk,idiosyncraticRisk); % default rate
RCf_HP_B_RR   = CapitalRequirementAlternativeHP(RecRate_sim_freq,DefRate_mean,...
   rho_Basel,CL,systematicRisk,idiosyncraticRisk);       % recovery rate
RCf_HP_B_d_RR = CapitalRequirementAlternativeHP(RecRate_sim_freq,DefRate_sim_freq,...  
   rho_sim_B_freq,CL,systematicRisk,idiosyncraticRisk); % default and recovery
...
d_sim_bayes  = samplingFromPosterior(N_sim,xd_vect,d_pdf);   % simulated d from posterior
...
rho_sim_bayes = samplingFromPosterior(N_sim,xrho_vect,rho_pdf); % simulated rho
...
disp('Large Homogeneous Portfolio')
% Computing Regulatory Capitals simulating different parameters LHP CL
RCb_LHP_d   = CapitalRequirementAlternativeLHP(RecRate_mean,DefRate_sim_bayes,...
    rho_mean,CL,systematicRisk);                % default rate
RCb_LHP_rho = CapitalRequirementAlternativeLHP(RecRate_mean,DefRate_mean,...
    rho_sim_bayes,CL,systematicRisk);               % correlation
RCb_LHP_all = CapitalRequirementAlternativeLHP(RecRate_mean,DefRate_sim_bayes,...
    rho_sim_bayes,CL,systematicRisk);               % default rate, correlation
...
disp('Homogeneous Portfolio')
% Computing Regulatory Capitals simulating different parameters HP CL
RCb_HP_d   = CapitalRequirementAlternativeHP(RecRate_mean,DefRate_sim_bayes,...
    rho_mean,CL,systematicRisk,idiosyncraticRisk);  % default rate
RCb_HP_rho = CapitalRequirementAlternativeHP(RecRate_mean,DefRate_mean,...
    rho_sim_bayes,CL,systematicRisk,idiosyncraticRisk); % correlation
RCb_HP_all = CapitalRequirementAlternativeHP(RecRate_mean,DefRate_sim_bayes,...
    rho_sim_bayes,CL,systematicRisk,idiosyncraticRisk); % default rate, correlation
...
posterior_d_CramerRao    = posteriorDistributionD(d_hat,d_vect,d_CramerRao_std); % posterior distribution d
 ...
 % distribution of X - default barriers     
xd_vect = linspace(-5,5,1000);              
f_x_d_CramerRao = @(x) normpdf(x,d_vect,d_std);           % density of x given mean(unknown) and std
d_pdf_CramerRao = bayesianPrediction(xd_vect,d_vect,posterior_d_CramerRao,f_x_d_CramerRao); % density of X conditioned on data

d_sim_bates_CramerRao  = samplingFromPosterior(N_sim,xd_vect,d_pdf_CramerRao);   % simulated d from its density
DefRate_sim_bayes_CramerRao = normcdf(d_sim_bates_CramerRao);                  % simulated default rate

   %  ii. Of correlation rho
rho_vect  = linspace(0.005,0.995,1980);         % equispaced column vector
rho_CramerRao_std = CramerRao_rho(10891,rho_vect,30);   % Cramer Rao stdev 
...
rho_tmp = samplingFromPosterior(n_rho,xrho_vect,rho_pdf_CramerRao); 
...
% Computing Regulatory Capitals simulating different parameters HP CL
RCb_HP_d_CR = CapitalRequirementAlternativeHP(RecRate_mean,DefRate_sim_bayes_CramerRao,...
    rho_mean,CL,systematicRisk,idiosyncraticRisk);    % default rate
RCb_HP_rho_CR = CapitalRequirementAlternativeHP(RecRate_mean,DefRate_mean,...
    rho_sim_bayes_CramerRao,CL,systematicRisk,idiosyncraticRisk); % correlation
RC_HP_allB_CR = CapitalRequirementAlternativeHP(RecRate_mean,DR_nested,...
